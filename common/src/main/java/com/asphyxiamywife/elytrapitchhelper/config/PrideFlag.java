...
public enum PrideFlag {
    ...
    AROMANTIC("aromantic", "option.elytrapitchhelper.pride_flag.aromantic",
    ...
    public static int colorAt(int[] colors, double position) {
        ...
        int index = (int) Math.floor(MathUtil.clamp(position, 0.0, 0.999999) * colors.length);
        return colors[MathUtil.clamp(index, 0, colors.length - 1)] & 0x00FFFFFF;
    }
